...
| 6   | `registerMessageRenderer(customType, renderer)`                | `piweb.registerMessageRenderer(customType, (message, opts) => tree)`           |   ✅   | Renderer returns a serializable node tree (Stack/Row/Text/Button/Frame + a new `Code` node) instead of a live `Component`. Custom messages (`pi.sendMessage({ customType })`) render via the tree on live `message_end` + replay; unrendered types fall back to markdown. Auto-routing fenced code blocks through a synthetic `"code"` type is the rest of TODO #19. |
