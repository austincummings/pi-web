...
export function createPiWebHost({ broadcastPanels, getPi }) {
  ...
  const renderPanel = (p) => {
    try {
      ...
      return { id: p.id, title: p.title, tree: { type: "Text", text: `render error: ${err.message}` } };
    }
  };
  ...
  const host = {
    ...
    registerPanel(id, def = {}) {
      panels.set(id, {
        id,
        title: def.title ?? id,
        render: typeof def.render === "function" ? def.render : () => ({ type: "Text", text: id }),
        actions: def.actions ?? {},
    ...
    async dispatch(panelId, action, payload) {
      ...
      const ctx = {
        ...
        setState(patch) {
          const next = typeof patch === "function" ? patch(p.state) : patch;
      ...
      try {
        ...
        console.error(`[piweb] action ${panelId}.${action} failed:`, err);
      }
      push();
    },
  };
